    ...
    RemoteEventListener		// So we can receive chat notifications
{
   ...
   */
  private static final String versionString = "fki-7.0";

  /**
   * Holds the Jini ServiceItems of found ChatServers.
   */
  private final Vector<ServiceItem> servers = new Vector<> ();

  /**
   * Maps from Jini ServiceID to ServiceItem.
   */
  private HashMap<ServiceID,ServiceItem> serverIDs = new HashMap<> ();

  /**
   * Refers to the service object of the currently connected chat-service.
   */
  private ChatServerInterface myServer = null;
   ...
   * want to find.
   */
  private static final String csi = "chat.server.ChatServerInterface";
      ...
      RemoteException
  {
    ...
    // service.
     ...
     ServiceTemplate chatServiceTemplate =
...
//      RemoteException
   ...
   */
  private void disconnect (ChatServerInterface server)
  {
    if (server != null) {
      try {
   ...
   */
  private void connectToChat (String serverNamePattern)
  {
   ...
   */
  private void listServers (boolean purge) {
    if (servers.isEmpty ()) {
   ...
   */
  private void showHelp (String [] argv) {
    System.out.println ("[" + versionString + "]");
      for (String aCmdHelp : cmdHelp) {
          System.out.println("[" + aCmdHelp + "]");
      }
  }
   ...
   */
  private String stringJoin (String [] sa, int firstIndex, String delimiter) {
