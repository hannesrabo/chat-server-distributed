    ...
    Runnable			// for the distribution thread.
{
   ...
   * modified.
   */
  private final Vector<RemoteEventListener> clients = new Vector<> ();
   ...
   */
  private JoinManager jmgr;

  /**
   * The delivery thread runs while this flag is true.
   */
  private boolean runDelivery = true;
...
//      UnknownHostException	// if we don't know where we are
  {
    ...
    ServiceIDListener sidListener = sid -> System.out.println("Registered as a Jini service " + sid);
   ...
   */
  private void addClient (RemoteEventListener rel) {
   ...
   */
  private void removeClient (RemoteEventListener rel) {
    synchronized (clients) {
      clients.remove (rel);
    }
